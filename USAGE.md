# NQ期货回测系统 - 使用说明

## 📖 快速开始

### 最简单的用法
```bash
python backtrader_demo.py
```
这将使用默认参数运行：2024年全年数据，1分钟K线

## 🎯 命令行参数

### 参数列表

| 参数 | 类型 | 说明 | 默认值 |
|------|------|------|--------|
| `--start` | 日期 | 开始日期（YYYY-MM-DD） | 2024-01-01 |
| `--end` | 日期 | 结束日期（YYYY-MM-DD） | 2024-12-31 |
| `--timeframe` | 选项 | K线周期 | m1 |
| `--data` | 路径 | 数据文件路径 | nq_m1_cleaned.csv |
| `--save-trades` | 开关 | 保存交易记录到CSV | （不保存） |
| `--plot` | 开关 | 生成可视化图表 | （不生成） |

### K线周期选项

| 值 | 说明 | 推荐场景 |
|----|------|----------|
| `m1` | 1分钟 | 高频交易、超短线 |
| `m5` | 5分钟 | 日内短线 |
| `m15` | 15分钟 | 日内波段 |
| `m30` | 30分钟 | 日内波段 |
| `h1` | 1小时 | 短期趋势 |
| `h4` | 4小时 | 中期趋势 |
| `d1` | 日线 | 长期趋势 |

## 💡 使用示例

### 1. 基础示例

#### 默认运行
```bash
python backtrader_demo.py
```

#### 指定时间范围
```bash
python backtrader_demo.py --start 2024-01-01 --end 2024-06-30
```

#### 使用不同K线周期
```bash
# 5分钟K线
python backtrader_demo.py --timeframe m5

# 1小时K线
python backtrader_demo.py --timeframe h1

# 日线
python backtrader_demo.py --timeframe d1
```

### 2. 保存交易记录

```bash
# 保存交易记录到CSV
python backtrader_demo.py --timeframe h1 --save-trades

# 指定时间范围并保存
python backtrader_demo.py --timeframe m5 --start 2024-01-01 --end 2024-06-30 --save-trades
```

交易记录保存位置：`backtest_results/trades_*.csv`

### 3. 生成图表

```bash
# 生成可视化图表
python backtrader_demo.py --timeframe d1 --plot

# 指定时间范围并生成图表
python backtrader_demo.py --timeframe h4 --start 2024-01-01 --end 2024-12-31 --plot
```

图表将显示在窗口中，关闭窗口后程序继续执行。

### 4. 完整功能组合

```bash
# 保存记录 + 生成图表
python backtrader_demo.py --timeframe h1 --save-trades --plot

# 完整参数示例
python backtrader_demo.py \
  --start 2024-01-01 \
  --end 2024-06-30 \
  --timeframe h4 \
  --data nq_m1_cleaned.csv \
  --save-trades \
  --plot
```

### 5. 特定场景示例

#### 测试最近一个月（快速验证）
```bash
python backtrader_demo.py --timeframe d1 --start 2024-11-01 --end 2024-12-31
```

#### 测试2024年上半年（详细分析）
```bash
python backtrader_demo.py --timeframe h1 --start 2024-01-01 --end 2024-06-30 --save-trades --plot
```

#### 测试单个季度（日线）
```bash
python backtrader_demo.py --timeframe d1 --start 2024-01-01 --end 2024-03-31 --plot
```

#### 测试一周数据（5分钟线）
```bash
python backtrader_demo.py --timeframe m5 --start 2024-12-10 --end 2024-12-17 --save-trades
```

## 📊 输出说明

### 终端输出

运行后会显示：
```
============================================================
NQ期货回测系统
============================================================

数据文件: /Users/hong/PycharmProjects/prepareQD/nq_m1_cleaned.csv
正在加载清洗后的数据...
回测时间范围: 2024-01-01 到 2024-12-31
K线周期: 1小时
✅ 数据已重采样为 1小时
✅ 数据加载完成
策略参数: 快线=10, 慢线=30

============================================================
开始回测...
初始资金: $100,000.00
============================================================

============================================================
回测结果
============================================================
最终资金: $163,824.04
总收益: $63,824.04
收益率: 63.82%

--- 分析器结果 ---
夏普比率: N/A
最大回撤: 98.91%
最长回撤期: 275000 bars

总交易次数: 2177
盈利交易: 527
亏损交易: 1649
平均盈利: $7,396.40
平均亏损: -$2,325.08

--- 自定义分析 ---
交易总数: 2176
胜率: 24.22%
平均盈亏: $29.34
总盈亏: $63,837.17

✅ 回测完成！
```

### CSV文件输出（使用 --save-trades）

文件保存在：`backtest_results/trades_{timeframe}_{start}_{end}_{timestamp}.csv`

包含18个字段：
- 基础信息：trade_id, datetime, type, price
- 交易细节：size, value, commission
- 技术指标：sma_fast, sma_slow, rsi, macd, macd_signal, atr
- 账户状态：portfolio_value, cash, position
- 盈亏分析：pnl, pnl_percent

详细说明请查看：`CSV_FILES_GUIDE.md`

### 图表输出（使用 --plot）

图表包含：
- K线图（红涨绿跌）
- 成交量
- 技术指标（SMA、RSI、MACD、布林带）
- 买卖信号标记
- 账户价值曲线
- 回撤曲线

## 🔧 高级用法

### 查看帮助信息
```bash
python backtrader_demo.py --help
```

### 使用自定义数据文件
```bash
python backtrader_demo.py --data /path/to/your/data.csv --timeframe h1
```

### 批量测试不同周期
```bash
#!/bin/bash
# 创建批量测试脚本 test_all_timeframes.sh

for tf in m5 m15 m30 h1 h4 d1; do
    echo "Testing timeframe: $tf"
    python backtrader_demo.py --timeframe $tf --start 2024-01-01 --end 2024-12-31 --save-trades
done
```

### Python脚本调用
```python
import subprocess

# 运行回测
result = subprocess.run([
    'python', 'backtrader_demo.py',
    '--timeframe', 'h1',
    '--start', '2024-01-01',
    '--end', '2024-06-30',
    '--save-trades'
], capture_output=True, text=True)

print(result.stdout)
```

## ⚠️ 注意事项

### 数据文件
- 确保 `nq_m1_cleaned.csv` 文件存在
- 如果没有清洗过数据，先运行数据清洗（见下文）

### 时间范围
- 开始日期必须早于结束日期
- 日期格式必须是 YYYY-MM-DD
- 确保日期在数据范围内（2020-01-01 到 2025-12-19）

### K线周期
- 较小的周期（m1、m5）数据量大，运行时间长
- 建议先用小范围数据测试
- 日线（d1）适合长时间范围测试

### 图表显示
- 需要安装matplotlib：`pip install matplotlib`
- 图表窗口需要手动关闭才能继续程序
- 如果无法显示图表，检查图形环境配置

## 📚 相关文档

- `README_backtrader.md` - 完整说明文档
- `QUICK_START.md` - 快速开始指南
- `CSV_FILES_GUIDE.md` - CSV文件说明
- `FEATURES.md` - 功能特性详解
- `USAGE.md` - 本文件（使用说明）

## 🛠️ 常见问题

### Q1: 如何数据清洗？
**A**: 数据已清洗完成（`nq_m1_cleaned.csv`），如需重新清洗：
```bash
python clean_nq_data.py
```

### Q2: 回测太慢怎么办？
**A**: 
- 使用较大的K线周期（h1、d1）
- 减小时间范围
- 关闭图表生成（不使用--plot）

### Q3: 如何查看交易记录？
**A**: 
```bash
# 查看最新的交易记录
ls -lt backtest_results/ | head -5

# 用pandas查看
python -c "import pandas as pd; df=pd.read_csv('backtest_results/trades_*.csv'); print(df.head())"
```

### Q4: 如何修改策略参数？
**A**: 编辑 `backtrader_demo.py` 中的 `strategy_params` 字典

### Q5: 支持哪些数据格式？
**A**: CSV格式，需包含：ts_event, open, high, low, close, volume

### Q6: 可以实盘交易吗？
**A**: 当前仅支持回测，实盘需要连接交易接口（未实现）

## 💻 系统要求

- Python 3.7+
- backtrader
- pandas
- matplotlib（可选，用于图表）

安装依赖：
```bash
pip install backtrader pandas matplotlib
```

## 📞 技术支持

如有问题，请查看：
1. 运行 `python backtrader_demo.py --help` 查看参数说明
2. 查看错误信息和堆栈跟踪
3. 检查数据文件是否存在且格式正确
4. 确认日期范围和参数格式

---

**版本**: v2.0  
**最后更新**: 2024-12-21

